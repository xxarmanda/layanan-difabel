...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'create') {
            ...
            $stmt = $conn->prepare("INSERT INTO pilihan2_posts (title, text, image, url) VALUES (?, ?, ?, ?)");
            ...
            $stmt = $conn->prepare("UPDATE pilihan2_posts SET title = ?, text = ?, image = ?, url = ? WHERE id = ?");
            ...
            $stmt = $conn->prepare("DELETE FROM pilihan2_posts WHERE id = ?");
...
$result = $conn->query("SELECT * FROM pilihan2_posts ORDER BY created_at DESC");
?>
...
<head>
    ...
    <title>Admin Pilihan2</title>
    <link rel="stylesheet" href="{{ asset('css/adminn.css') }}">

</head>
<body>
    <h1>Manage Postingan Tunawicara</h1>

    <!-- Create New Post Form -->
    <form action="{{ route('pilihan2.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
    ...
    <!-- Display All Posts -->
    <h2>Semua Postingan Tunawicara</h2>
    @if ($posts->count())
        @foreach ($posts as $post)
            <section class="content">
                ...
                <form action="{{ route('pilihan2.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                ...
                <form action="{{ route('pilihan2.destroy', $post->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
